...
async def main(args):
    sender = UDPSender(args.plugin_ip, args.plugin_port)
    log.info(f"UDP target: {args.plugin_ip}:{args.plugin_port}")
    ...
    clients = []
    if pfd_mac:
        clients.append(BezelClient(pfd_mac, "PFD", sender))
    if mfd_mac:
        clients.append(BezelClient(mfd_mac, "MFD", sender))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--plugin-port', type=int, default=DEFAULT_PLUGIN_PORT,
                        help=f'Plugin UDP port (default: {DEFAULT_PLUGIN_PORT})')
